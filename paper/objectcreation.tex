...
object $o_h$, such that (1) for all $o' <_h o$ the object $o'$ is allocated (i.e., its \codein{<allocated>} field is set to true), and
...
object $o_h^C$, where (1) for all $o' <^C_h o$ the object $o'$ is allocated (i.e., its \codein{<allocated>} field is set to true), and
...
For each class $C$, we introduce a unary function symbol $C::$\codein{allocate} with the signature $\mathtt{Heap} \rightarrow \mathtt{Object}$,
...
\end{align*}
\end{definition}
The first rule uses the assumption that the orders are only sensitive to the \codein{<allocated>} field and the second one that they are indendent, besides the subtyping relation.
