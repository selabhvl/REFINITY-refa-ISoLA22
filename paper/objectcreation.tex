...
\EK{
\paragraph{Approach.}
To formalize this assumption, we first need to express the determinism of the object creation.
}
...
To so so, each object has a field \codein{<allocated>} that is set to \codein{true} for all allocated objects, and to \codein{false} for all others.
}
...
An update is a syntactic representation of a subsitution on a program variable. A simple update, which is the only form of update we require here, 
...
\EK{
Semantically, JavaDL is evaluated over a Kripke Structure and an interpretation $I$. The interpretation assigns value to predicate and function symboles, 
...
according to the semantics of the program. The semantics of an update is the transition from one state to another, according to the subsitution expressed by it.
The sequent calculus for JavaDL realizes symbolic execution by reducing modalities to updates (under certain side-condition, added to the premise/path-condition).
}
...
We adapt the previous definition to take into account the types of objects, \EK{by having a \emph{set} of orders $<_h$, one for each class.}
...
object $o_h^C$, such that (1) for all $o' <^C_h o_h^C$ the object $o'$ is allocated (i.e., its \codein{<allocated>} field is set to true), and
...
If~\codein{C} is a sub type of~\codein{D}, then the proof fails.
