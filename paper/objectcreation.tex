...
We stress that the taclets for the different options are not \emph{unsound}, but merely switch between different version of assumptions about the (in this case underspecified) Java object model. \EK{Of course, the default version is not using the new taclets and stick with the originals, i.e., not assuming deterministic allocation.}
