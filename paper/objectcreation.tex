...
object \vsnote*{@EK: forgot to use $o_h$?}{$o_h$}, such that (1) for all $o' <_h o$ the object $o'$ is allocated (i.e., its \codein{<allocated>} field is set to true), and
...
\end{definition}
The modification \vsnote*{@EK?}{of KeY's rule \texttt{allocateInstance}} is the addition of $v = \mathtt{allocate(heap)}$ to the antecedent.

The modified rule suffices to show the simple equivalence from above.\vsnote{@EK: Not directly, since we still need the 2nd def.? Rephrase?}
...
They are not equal in the sense that, if there is a global \vsnote*{Wouldn't call this implicit, quite the contrary?}{\sout{(implicit)}} counter
...
They are, however, distinguishable in the proof system \vsnote*{}{due to the term-representation of the heap?}.
...
\vsnote*{Added. @EK Please check -- also for relevance!}{%
Note that this rule is still unaware of intermittent operations that are symbolically represented on the heap, e.g.\ in \lstinline{x = new C(); x.set(1); y = new D();} versus \lstinline{y = new D(); x = new C(); x.set(1);}.
}
...
Second, while we only discussed object creation here, \vsnote*{@EK: what?}{it} is crucial for \emph{exceptions}, which must be created before being thrown. 
