...
The specification facilities for either Java constructs or abstract program fragments are described in the Java Modelling Language (JML) with some extensions.
...
In our case we use the default postcondition REFINITY provides which is simply \rcode{\\result\_1 == \\result\_2} which are two sequences that contain for each respective side in this order:
The return value if any, otherwise null; exceptions thrown if any, otherwise null; onwards, the final values at the relevant abstract locations specified.

Thus the default postcondition in our case will be that \rcode{x} returned in Before must be identical to \rcode{temp} returned in After and that any exception thrown must be identical for the two sides.
...
We get several instances of SE resulting in the reference \rcode{x} holds being changed by the abstract statement \rcode{N} in \rcode{n()}.
