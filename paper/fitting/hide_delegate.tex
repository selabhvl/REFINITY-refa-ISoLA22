...
\OA{
Also of note is the scenario shown in the sequence diagram in Fig.~\ref{fig:NPEs} where we assume the call to \jcode{f()} returns \jcode{null} causing the call to \jcode{g()} to result in a \jcode{NullPointerException} (NPE)}. In the strictest sense of behavioral preservation
