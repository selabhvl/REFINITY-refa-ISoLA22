...
We note that a pattern usually considered is also \jcode{X x = o.f(); Y y = x.g()} potentially with non-interfering intermediate statements in between the two statements,
...
\OA{
Note the scenario shown in the sequence diagram in Fig.~\ref{fig:NPEs} where we assume the call to \jcode{f()} returns \jcode{null} causing the call to \jcode{g()} to result in a \jcode{NullPointerException} (NPE)}. In the strictest sense of behavioral preservation
...
In order to prove the specified \rname{Hide Delegate} refactoring in the published version (v0.9.7) of \Refinity{} one must write a postcondition that consists of a conjunction of return values of the before and after programs being identical and
that exceptions thrown are both instances of NPE or equal.
This is owing to the fact that \Refinity{} does not consider occurences of \jcode{new NullPointerException()}, or any other newly created objects, to be equal.

\OA{We have produced a modified version of \Refinity{} where this is no longer an issue such that we may keep the default postcondition that simply matches return results and exceptions:
\Refinity{} will automatically manage prove the shown \rname{Hide Delegate} refactoring to be correct wrt. the given postconditions. In the following section we will detail the changes need to accomplish this.}
