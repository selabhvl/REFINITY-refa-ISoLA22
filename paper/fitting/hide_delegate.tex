...
\vsnote{Rephrase to active voice: ``The scenario ... will cause an NPE when the call to f returns null.}
\OA{
...
Note that we \vsnote*{Double negation: ``allow''}{do not require} that the abstract statements \rcode{F} and \rcode{G} in the methods \rcode{getOwner()} and \rcode{getResource} \vsnote*{}{\sout{cannot}} complete abruptly.
...
In order to prove the specified \rname{Hide Delegate} refactoring in the published version (v0.9.7) of \Refinity{} we need a postcondition that consists of a conjunction of return values of the before- and after-programs being identical and
...
\OA{We have \vsnote*{}{improved \sout{produced a modified version of}} \Refinity{} where this is no longer an issue such that we may keep the default postcondition that simply matches return results and exceptions:
\Refinity{} automatically manages to prove the shown \rname{Hide Delegate} refactoring to be correct wrt. the given postconditions. In the following section we will detail the changes needed to accomplish this.}
