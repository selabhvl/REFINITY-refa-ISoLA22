...
The Hide Delegate refactoring can be described as an Extract Method refactoring on a call chain such as \lstinline[style=smallJava]|Y y = o.f().g()|, where
the call chain is extracted to a method on \lstinline[style=smallJava]|o| such that we can replace the chain with a call to the new method \lstinline[style=smallJava]|o.h()|, 
which we declare as \lstinline[style=smallJava]|Y h(){ return this.f().g(); }|.
The refactoring can enable less
coupling as the class that contained the call chain afterwards does not need to know the return type of \lstinline[style=smallJava]|f()|.
