...
A refactoring proof is a relational verification proof that compares two programs, \vsnote*{TODO: We'll have to rephrase this, I think.}{which have the \emph{same} abstract statements}.
...
\vsnote{Note to self: technically our encoding in R. doesn't actually move anything ;-)}
...
and how this information can be used in dynamic logic proofs. The different possibilities are implemented as different rule sets and \vsnote*{@EK? Unclear to VS. ``Cannot''? Or?}{must not} be encoded in the relational post-condition.
\vsnote*{Weird phrasing, rewrite? Isn't reducing size of the spec a good thing?}{%
  This reduces the size of the required specification, a notorious bottleneck} in formal verification~\cite{DBLP:journals/corr/abs-1211-6186,DBLP:series/lncs/HahnleH19}.


Furthermore, we discuss the necessary extensions needed to prove equivalent behaviour where one data structure is replaced by another, e.g., an array or any primitive type by a \vsnote*{}{class \sout{special record}}.
...
We first describe AE and relational verification using the \rname{Extract Local Variable} refactoring in Sec.~\ref{sec:prelim}, before we discuss the challenges for AE in Sec.~\ref{sec:challenges}. Sec.~\ref{sec:hide} describes and proves the \rname{Hide Delegate} refactoring(s). Sec.~\ref{sec:discussion} discusses our results and Sec.~\ref{sec:related} gives the related work. Lastly, Sec.~\ref{sec:conclusion} concludes.
