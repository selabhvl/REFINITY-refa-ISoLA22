Refactoring is a fundamental activity in software engineering to reorganize code while preserving its observable behavior, 
...
Consider relating\\
\begin{center}
\jmcode{if (E$_{\mathit{boolean}}$) \{S$_1$;\} else \{S$_2$;\} return;}\\
and\\
\jmcode{if (!E$_{\mathit{boolean}}$) \{S$_2$; return;\} S$_1$; return;}
\end{center}
where \jmcode{E$_{\mathit{boolean}}$} represents an arbitrary boolean expression, \jmcode{S$_1$} and \jmcode{S$_2$} represent arbitrary statements.
...
To investigate the use of AE for refactorings on the class-level, we investigate the \rname{Hide Delegate} refactoring that moves code between classes and show how it can be encoded in \Refinity{}.
