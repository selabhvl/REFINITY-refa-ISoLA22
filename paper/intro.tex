\EK{Refactoring is a fundamental activity in software engineering to reorganize code while preserving its observable behavior, 
but improving the structure of the code, e.g., to simplify maintenance.
...
Relational verification through dynamic logic is a promising approach for verification of refactoring patterns in object oriented programs.
...
Consider relating ``\jmcode{if ($E_{boolean}$) \{$S_1$;\} else \{$S_2;$\}return;}'' and ``\jmcode{if(!$E_{\mathit{boolean}}$) \{$S_2$;return;\} $S_1$;return;}'' where
$E_{\mathit{boolean}}$ represents an arbitrary boolean expression, $S_1$ and $S_2$ represent arbitrary statements.
One of the programs represents the schema of the code before the refactoring, and the other one the code afterwards, with respect to some relational post-condition that defines the notion of \emph{program equivalence}. \EK{This reasoning about abstract programs is exactly the aforementioned reasoning about the program transformation behind the refactoring.}

\EK{Despite the promising results due to AE, it has only been applied to \emph{statement-level} refactorings, that
...
\EK{Following the general approach of AE,} \Refinity{} \cite{steinhoefel:ae}, the tool supporting verification on top of the KeY system, an automated theorem prover for Java~\cite{DBLP:conf/aplas/Steinhofel20},
...
\EK{To investigate the use of AE for refactorings on the class-level, we investigate the \rname{Hide Delegate} refactoring that moves code between classes and show how it can be encoded in \Refinity{}.
...
This issue goes beyond exceptions, but touches on a fundamental problem:
there are several possible choices for when newly created objects (and exceptions, which are objects in Java) are considered equal in relational verification using dynamic logic.
...
and how this information can be used in dynamic logic proofs. The different possibilities are implemented as \vsnote*{Unclear, at least that early in the paper. Why ``different''? Also see my later point about why using the new rule(s) should be optional.}{different rule sets} and do not have to be encoded explicitly in the relational post-condition.
...
We first describe AE and relational verification using the \rname{Extract Local Variable} refactoring in Sec.~\ref{sec:prelim}, before we show necessary conditions for the \rname{Hide Delegate} refactoring to be correct  and then discuss the challenges for AE in Sec.~\ref{sec:challenges}.
