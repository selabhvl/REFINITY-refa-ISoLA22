\EK{Refactoring is a fundamental activity in software engineering to reorganize code while preserveing its observable behavior, with respect to some understanding of presevance, 
but improving some other property of the code, e.g., to simplify maintenance.
A refactoring can be defined as a pattern it matches on and a subsequent program transformation on the program it matched on.
...
\EK{To investigate the use of AE for refactorings on class-level, we investigate the \rname{Hide Delegate} refactoring that moves code between classes and show how it can be encoded in \Refinity{}.
...
This issue goes beyond exceptions, put touches on a fundamental problem:
There are several possible choices for when newly created objects (here: exceptions, which are objects in Java) are considered equal in relational verification using dynamic logic.
