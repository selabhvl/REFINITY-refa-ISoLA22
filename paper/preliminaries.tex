...
Semantically JavaDL formulas are evaluated in a Kripke structure; a collection of first-order structures \cite[sec. 3.3]{DBLP:series/lncs/10001}.
...
\OA{We summarize how AE is implemented in \Refinity{} by showing how to specify a method \rcode{void n()} with a mostly unknown method body.
...
A method is specified and its only content is an abstract statement \rcode{\\abstract\_statement N} preceded by a Java Modelling Language (JML) like\footnote{We say ``like'' as JML does not deal with abstract Java programs.} specification~\cite{HuismanAhrendtBruns2014_1000041881}.
...
An abstract location set represents a fixed set of memory locations that a (part of a) program may read or write to.
