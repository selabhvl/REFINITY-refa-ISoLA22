...
A method is specified and its only content is an abstract statement \rcode{\\abstract\_statement N;} preceded by a JML specification~\cite{HuismanAhrendtBruns2014_1000041881} for it.
embedded in the comment (every line in the comment beginning with a \rcode{@}-character).
The specification is straightforward with no surprises: Abstract statement \rcode{N} may assign to the
abstract location set \rcode{frN}, access the abstract location set \rcode{fpN} and may not throw any exceptions.}
