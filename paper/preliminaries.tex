...
We specify it can possibly assign to some abstract locations (its frame) and access some abstract locations (its footprint).
Furthermore we specify that no exceptions will be thrown by the method body.}
...
A method is specified and its only content is an abstract statement \rcode{\\abstract\_statement N;} preceded by a comment where every line in the
...
The specification reads straightforward with no surprises. An execution of the abstract statement \rcode{N} may have the effects of: Assigning to the
