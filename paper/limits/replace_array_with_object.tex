Let us consider a refactoring that replaces an array containing a geographical position given by a latitude and longitude as in Listing~\ref{refa:ReplaceArray-java-before} with an object that gives read or write access to the same values through setters and getters that make it immediately clear what is being accessed as seen in Listing~\ref{refa:ReplaceArray-java-after}.
...
We can then formulate the question to REFINITY as given in Fig.~\ref{refa:ReplaceArray-REF} for each of the operations in the API, here the matching pairs of setters/getters:
...
\vsnote*{}{After spelling this out, it looks like we should just start with Encapsulate Value or how it is called.}
